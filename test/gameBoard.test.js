...
describe("GameBoard", () => {
  ...
  test("can assign ships to specific coordinates", () => {
    warZone.assignShip(5, 4, 4, "horizontal");
  ...
  test("can deploy ship of given length on the board", () => {
    const startx = 5,
      ...
      length = 4,
      direction = "vertical";
    warZone.assignShip(startx, starty, length, direction);
  ...
  // No need, it will assign the ship in nearest place instead of rejection to facilitate Random Placement

  // test("GameBoard can identify already assigned cell and reject reassignment", () => {
  //   let toSucess = warZone.assignShip(5, 4, 3, "horizontal");

  //   let toFail = warZone.assignShip(5, 4, 3, "horizontal");
  ...
  test("Gameboard is able to process a attack on a ship", () => {
    warZone.assignShip(4, 3, 3, "horizontal");
  ...
  test("Check if all ships are sunk and game is over", () => {
    warZone.assignShip(4, 4, 2, "horizontal");
    warZone.assignShip(4, 1, 3, "horizontal");
    warZone.receiveAttack(4, 4);
    warZone.receiveAttack(4, 5);
